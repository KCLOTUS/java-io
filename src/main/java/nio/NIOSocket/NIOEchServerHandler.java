 ...
 * @version 1.0.0
 */
public class NIOEchServerHandler implements Runnable {
     ...
     * 初始化多路复用器，绑定监听接口
     */
    public NIOEchServerHandler(int port){
        try {
            //读取队列大小
            int backLog = 1024;
    ...
    public void run() {
        ...
        //多路复用器关闭后，所有注册在上面的Channel和Pipe等资源都会被自动注册关闭，所以不需要重复释放资源
        if (selector != null){
            try {
                selector.close();
            }catch (IOException e){
